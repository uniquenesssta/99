...
> 新修复执行入口：[全链路一致性修复任务书](HFM_CHAIN_CONSISTENCY_REPAIR_TASKBOOK.md)。R-01～R-04自动验证通过、原生/实机缺口单列；R-05标签意图与确认生命周期自动验证通过待实机（111/111），见该书§20；R-06/R-07未开始。此前审计中的日志最后建议已被用户当前顺序取代。
