...
> 2026-09-26 常驻 DirectWrite 试验：[任务书 §15](HFM_RESIDENT_DIRECTWRITE_PREVIEW_TASKBOOK.md#15-dw-05-执行卡2026-09-26)。DW-01～DW-05 已实现并通过独立 Windows 原生自动门（含受控 SMB）；未安装字体列表/详情可用 `npm run dev:dw` 显式试用，默认 `npm run dev` 保持原后端。原 CIM 1500ms 问题已替换为原生 Unicode 映射查询并通过真实 Windows 门，综合 Windows 全量诊断/构建/混淆及 Windows/Linux 原生回归全部通过（Actions `36244288794`）；下一阶段 DW-06 故障与退出收口尚未开始。DW-00 实机基线、DW-07 用户 NAS/图像/性能仍待验；其他一般性能优化暂停，55 字体解析未解决，不覆盖 C-09/O-07。
...
> 新修复执行入口：[全链路一致性修复任务书](HFM_CHAIN_CONSISTENCY_REPAIR_TASKBOOK.md)。R-01～R-04自动验证通过、原生/实机缺口单列；R-05标签意图与确认生命周期自动验证通过待实机（111/111），见该书§20；R-06完整提交身份与双通道去重自动验证通过待实机（112/112，原生缺口单列），见该书§21；R-07总验收自动验证通过待实机（113/113），见该书§22；六个原生入口因Cargo缺失未执行，Windows/NAS及历史性能待验项继续保留。此前审计中的日志最后建议已被用户当前顺序取代。
...
> 新实机修复入口：[索引访问、共享 I/O、激活清理与退出一致性修复任务书](HFM_INDEX_IO_ACTIVATION_SHUTDOWN_REPAIR_TASKBOOK.md)。C-00～C-07 已完成。C-05 ownership、激活入口回归和 C-05R Windows sharing violation 回收均已收口，Windows 实机激活链真实通过；C-06 已拆分退出三轴结果。C-07 新增 renderer 显式 closing lifecycle，复用现有 app-window close-flush 协议并在 close cancelled 后恢复，关闭后 developer/shared foreground/非必要 metrics 不再派生 IPC；main assertApplicationOpen 未放宽。Windows C-07 verification `35737665904` 全绿，C00 current 已降为 0 缺陷 / 8 对照。下一项 C-08 Shared I/O 性能，O-07 继续暂停。
...
状态：实现、自动门禁及 Windows 完整构建验收完成。统一拒绝策略、精确调度组键、版本化缓存兼容、限频日志及原生第二道校验已落地；Windows 75 项诊断、三后端输入校验、Rust release、Electron 三端构建及混淆通过。位图与内存外部验收保留，最新证据见 Stage 3 第 10.9 节。

Windows 补验反馈：PowerShell 孤立代理项校验、symlink 测试权限和 CRLF 诊断兼容问题均已解决。2026-09-08 用户明确快进到 `476c5d6`，随后 Windows 完整 build 成功：75 项诊断（含换行 24 场景、C++ 68、PowerShell 68、Rust shared fixtures、JS 190）、Rust release、公钥同步、Electron 三端和混淆 3/3 全部完成。Stage 3 没有 AT-3.3；下一项为 AT-4.1。外部矩阵与构建验收界限见 Stage 3 第 10.9 节。
...
当前实施、资源归属、八处延迟绑定及真实回调循环见 [Stage 4 任务书](HFM_STAGE_04_MAIN_COMPOSITION_TASKBOOK.md)。本项只建立契约并强化注册入口，尚未搬迁巨型文件实现。
...
2026-09-10 完成：入口 715→77 行，只显示分组依赖的创建、绑定、启动和注册。标签协调归入单一组合所有者；Application 按 lifecycle/query/mutation/maintenance/preview 分为 43/20/29/17/6 项，并适配原 115 项注册接口。31/65/118 个输入端口不变；逐项能力引用、八个错误接线/重复 start-stop 反例、旧生命周期和编排门禁通过，verify 80/80、三端构建与混淆通过。用户前置 Windows build 已成功，新版实机复验待补；详见 Stage 4 第 9.6、10 节。
...
- 状态：已完成实现与自动验证，分支 `stage/05-rust-worker-composition`，基于 Stage 4 `d316b44`。复审确认实际为 89 个公开类型、38 个内部命名 payload；运行时函数未修改，原 45 个方法/38 条命令、旧类型入口和领域语义保持。详见 [Stage 5 任务书](HFM_STAGE_05_RUST_WORKER_TASKBOOK.md)。
...
- 状态：实现及自动验证完成，基线 `e7b3d8a`；新 transport 283 行，门面 2195→1985 行。28 处临时文件由 transport 创建文件对象，在领域原 finally 点释放；369 个冻结命令用例、7 组状态/并发序列与真实子进程边界通过。详见 [Stage 5 第 6 节](HFM_STAGE_05_RUST_WORKER_TASKBOOK.md)。
...
- 状态：AT-5.3 已完成 5/5 组领域 client（38/38 条命令），各组独立提交；typecheck、83/83 项诊断、Electron 三端构建及混淆通过。AT-5.3 实现及自动验证完成；AT-5.4 收尾见对应章节。 详见 [Stage 5 第 7 节](HFM_STAGE_05_RUST_WORKER_TASKBOOK.md)。
...
- 状态：六组 AppRootViewProps 及直接子组件强类型已实现，18 个编译拒绝与四模式 UI 接线基线通过；typecheck、85/85 诊断、三端构建与混淆通过。详见 [Stage 6 任务书](HFM_STAGE_06_REACT_COMPOSITION_TASKBOOK.md)。
...
- 状态：已提取 16 个 state、7 个 ref 与 9 项只读派生；数据库分页、family、preview/selection 和滚动 effect 仍由原运行时唯一拥有。新增初始化/调用顺序/函数体/显式端口/真实 toolbar 与筛选行为门禁，typecheck、86/86 诊断和三端构建/混淆通过。详见 [Stage 6 任务书](HFM_STAGE_06_REACT_COMPOSITION_TASKBOOK.md)。
...
- 状态：已从 App 提取 42 个 state/ref，Operations/Library/Developer 分别独占 22/11/9；写队列、安装状态和开发诊断 raw refs 保持私有，数据库刷新只经命令/readonly ref，关闭 flush/确认顺序保持。新增真实生命周期、三项变异与 CRLF 门禁；typecheck、88/88 诊断、三端构建与混淆通过。用户已完成 Windows Cargo release、354/1/190 三端模块构建和混淆 3/3；GUI 硬门禁仍待复验。详见 [Stage 6 任务书](HFM_STAGE_06_REACT_COMPOSITION_TASKBOOK.md)。
...
| Stage 7 | AT-7.1 完成；AT-7.2 实现及自动验证完成 | `stage/07-ipc-security-dependencies`；AT-7.2 基线 `0810137` | IPC/导航精确边界；Electron 42 / builder 26 / electron-vite 5 / Vite 7；完整审计 23 -> 0；91/91 诊断与 build/混淆通过 | AT-7.2 Windows `npm ci`/审计 0 已收；首轮 CRLF 诊断问题已修，完整 build、原生 ABI、NSIS 与安装烟测待补 |
| Stage 8 | 阻塞于 Stage 7 | - | - | - |
