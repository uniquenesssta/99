...
> 2026-09-26 新试验任务书：[常驻 DirectWrite 预览](HFM_RESIDENT_DIRECTWRITE_PREVIEW_TASKBOOK.md)。DW-00 首批关联诊断已交付、实机待验；按用户授权完成 DW-01 原生最小实现及 DW-02 专用常驻服务/有界协议，独立 Windows 自动验证通过；默认后端保持不变，下一项 DW-03 对象复用，界面试用入口仍在 DW-05；其他一般性能优化暂停，55 字体解析与 CIM 失败单列。该试验不覆盖 C-09/O-07 验收。
