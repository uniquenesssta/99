...
> 新专项（O-00 基线已取证，业务未实施）：[共享离线保留与本地字体退出清理任务书](HFM_SHARED_OFFLINE_LOCAL_EXIT_TASKBOOK.md)。按用户最终要求保留并置灰离线共享目录/标签，恢复后校验启用；本地副本取消与退出不依赖网络，不建设离线同步系统。O-00 TypeScript 与 126/126 自动诊断通过（8 项已知缺口观察、5 组对照），Windows/NAS 实机待验；O-01～O-08 未开始，旧 U-09 与 Windows 待验项保持。
