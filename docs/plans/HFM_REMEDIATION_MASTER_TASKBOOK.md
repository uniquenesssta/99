...
- 当前阶段：Stage 1「激活、停用与托管安装事务正确性」执行中；AT-1.1 已完成，AT-1.2 待启动
...
| Stage 1 | 执行中（AT-1.1 完成） | 本阶段分支 AT-1.1 提交 | A1 正确性门禁与 `npm run verify` 通过，65/65 长期诊断；Electron/Vite 三端 build 通过 | 分支 `stage/01-activation-transactions`；AT-1.2 待启动，Windows 实机验收保留 |
| Stage 2 | 阻塞于 Stage 1 | - | - | - |
| Stage 3 | 阻塞于 Stage 2 | - | - | - |
| Stage 4 | 阻塞于 Stage 3 | - | - | - |
| Stage 5 | 阻塞于 Stage 3 | - | - | - |
