...
> 新专项（O-01 根状态与目录保留已实现）：[共享离线保留与本地字体退出清理任务书](HFM_SHARED_OFFLINE_LOCAL_EXIT_TASKBOOK.md)。按用户最终要求保留并置灰离线共享目录/标签，恢复后校验启用；本地副本取消与退出不依赖网络，不建设离线同步系统。O-00 TypeScript 与 126/126 自动诊断通过（8 项已知缺口观察、5 组对照），Windows/NAS 实机待验；O-01 已实现根状态/代次和本地确认目录，TypeScript、127/127 诊断、三端构建与混淆通过，Windows/NAS 待验；O-02 已接入共享元数据五项及安装状态读写、可回收目录探测，旧迁移/扫描等消费者仍未全部隔离，TypeScript、131/131 诊断及三端构建通过，完整门未通过（专项 §18.4～18.5）；用户明确要求先开始 O-03，现已实现置灰/整批准入，TypeScript、129/129、定向 40 组、三端构建及混淆通过，Windows/NAS 待验（见专项 §19）；O-02 前置仍未满足，O-04 已开始并建立 5 缺口/4 对照故障基线，受 O-02 及原生验证环境阻塞，尚未生产切换（见专项 §20）；O-05 已接入三份本地记录的安全持久化及删除队列并发保护，TypeScript、132/132、专项 7 组及 JS 三端构建通过，手动处置/文件身份/重启清理仍未完成（专项 §21）；O-06～O-08 未开始，旧 U-09 与 Windows 待验项保持。
