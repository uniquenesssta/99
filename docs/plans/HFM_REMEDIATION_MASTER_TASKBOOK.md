...
> 2026-09-26 新试验任务书：[常驻 DirectWrite 预览](HFM_RESIDENT_DIRECTWRITE_PREVIEW_TASKBOOK.md)。DW-00 首批关联诊断已交付、实机待验；按用户授权完成 DW-01 原生最小实现及 DW-02 专用常驻服务/有界协议，独立 Windows 自动验证通过；默认后端保持不变，DW-03 有界对象复用/释放与 DW-04 隔离字体副本已通过独立 Windows 验证（含受控 SMB）；下一项 DW-05 现有预览链接入及显式试用入口；其他一般性能优化暂停，55 字体解析与 CIM 失败单列。该试验不覆盖 C-09/O-07 验收。
