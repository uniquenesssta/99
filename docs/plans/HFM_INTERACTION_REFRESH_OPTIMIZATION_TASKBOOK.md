...
- 基线 `aebc063b1bfefd5851a645852ab375490b5770be`，沿用 `stage/09-preview-tags-app`。用户确认 §19 交互已达到预期；启动日志 `startup-2026-09-18_13-08-34-891-31688.log` 中标签页面 224/228ms 含提交后重新计算的 157/162ms grace，另有一次历史操作回放 `conflicts=1, changed=0`。本项不代表 U-09 或 U-08 性能验收完成。
- `tagMetadataRevisionBarrierRuntime` 增加显式提交标记；写协议成功返回及 worker/daemon 提交信号不再开启额外 220ms 查询等待，覆盖本地/共享、分页/ID 查询、空标签目录。未确认操作/错误仍保留 grace；版本递增、dirty 保护、缓存失效、在途旧结果拒绝与重读、统计每版本一次失效保持。
- `sharedTagOpsReplayRuntime` 的冲突仅统计同字体/标签当前最高版本的多条竞争操作。顺序添加/删除、跨电脑顺序编辑和已被更新版本解决的历史竞争不再报当前冲突；确定最终标签的排序规则不变。同版本同动作的不同操作仍保留竞争提示。
- 日志回放、详细诊断、前端冲突报告共用此判定。利用现有 meta 表记录诊断策略版本 2，已处理到最新操作的旧数据库也重新计算计数/样本；无新操作时仅更新诊断，不将历史操作重新覆盖到现有标签。清零时同步清除旧样本；诊断标记与操作回放同事务，失败整体回滚。不改数据库 schema、IPC、Rust、依赖版本或收藏/保护数据。
- 新增 `diagnostics:tag-commit-query`：执行真实写协议/信号/barrier/query facade，确定时钟验证提交后分页及 ID 路径零 grace、未确认/错误保留 grace、scope 隔离、空目录信号、并发新提交重读、缺版本回退、统计失效，并拒绝三个恢复旧逻辑的反例。共享回放原诊断新增真实 SQLite/原 schema 执行：正常历史、跨电脑编辑、当前竞争、后续解决、旧样本重算、较新元数据保护、回滚/重试；Git 基线确实复现正常增删误报。
- 验证：`npm run verify` 通过 TypeScript + 125/125 诊断；Electron/Vite 三端构建（367/1/200 模块）与混淆 3/3 通过，`git diff --check` 通过。没有调整冻结摘要或降低既有门禁。Windows/NAS 本轮未实测，用户更新后用 `npm run dev` 检查标签增删/最后标签删除和诊断面板。

### 20.1 关闭与断网的只读核查

- 正常退出已有：窗口先 flush（12 秒未回应提示返回/仍然关闭），窗口关闭后主进程清理临时激活和未完成补偿、删除临时文件、保存安装状态，再停止 watcher 与 worker。临时激活使用本机副本，清理不是依赖源字体继续在线；永久安装不属于该清理范围。
- `cleanupTemporaryActiveFontsUntilEmpty('quit')` 默认最多 18 轮，每轮仍有实际 I/O 与系统验证。未清完会恢复窗口并阻止退出；保存失败可选择仍然退出，但保存 promise 未返回时尚无统一退出期限。强杀/崩溃不等于清理成功，启动另有残留恢复；这份日志在关闭前已手动取消所有激活，不能用它证明带激活退出的实机结果。
- 共享预览有按根熔断（默认连续 3 次失败暂避 30 秒、10 次失败 120 秒），但这不覆盖共享标签、索引和全部退出链。安装状态 flush 还会等读取/写入、根定位与合并索引同步；部分共享元数据预检仍在主进程执行同步 SQLite，事件循环被网络文件操作阻塞时，单独加 JS 超时不能提供硬保证。
- 后续设计建议（本轮未实施）：断网停止新共享任务并隔离根；未提交标签等写入先以幂等操作身份落本地持久待办，恢复后再按版本补交；退出清理仅依赖本地激活记录与副本，网络同步不进入退出必经路径；设置整体退出期限，将可能阻塞的网络操作放在可终止的独立进程，保留失败清理记录下次恢复。不能把 Promise 超时当作底层 I/O 已取消，也不能把未确认的共享写入报成功。
- Mermaid Chart 已记录实际标签写入/回执/版本校验与共享回放链；未引入新第三方 API。Create State 沿用容量 2/2 后跳过的既定约定，以 Git 和本文交接。


## 21. 共享离线与退出专项衔接（仅任务书）

- 用户已进一步收窄 §20.1 的设计建议：共享监视文件夹和共享标签离线时保留原位置并置灰，重连后恢复；禁止离线共享修改，不做离线待办补交或离线同步系统。§20.1 关于保存并恢复后补交共享标签指令的建议不再执行。
- 本机激活继续使用本地托管副本；取消和退出去除源网络依赖，残留分阶段持久化并提供处置路径。网络结果未知只读核验，不自动重写；网络永远不恢复也必须能退出。
- 详细执行入口为[共享离线保留与本地字体退出清理任务书](HFM_SHARED_OFFLINE_LOCAL_EXIT_TASKBOOK.md)，O-00～O-08 全部未开始。本次只创建规划，不能视为离线或退出机制已修复，不改变 U-08/U-09 及其他实机待验状态。
