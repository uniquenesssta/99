...
- 验证：CI `35428328158` 最终 attempt 2 为 success。JS Linux 通过 `typecheck`、`window-close-flush`、`bounded-local-exit`、`react-composition-controllers`、Electron/Vite build 与混淆；JS Windows 通过 `typecheck`、`window-close-flush`、`bounded-local-exit`；Linux 原生 test/release 通过。Windows 原生首轮仅在未修改的 `local_tags_atomicity_delete_failure_is_atomic` 出现 `UNIQUE constraint failed: app_state.key`，对同一提交仅重跑该失败 job 后 test/release 通过，未弱化或跳过门禁。
- 高频调用量化补充：首个 Shared I/O 请求到退出前最后请求约 221.864 秒，7251 次调用约 32.68 次/秒；子进程存活中位数约 26ms、平均约 29.37ms、P95 约 49ms、最大 717ms。说明主要成本来自大量短命 one-shot worker 的进程创建与协议往返，而不是单次 I/O 本身很慢。
