...
const api = {
  ...
  activateFonts: (items: FontItem[], trace?: OperationTrace): Promise<FontActivationBatchResult> => invoke('fonts:activateFonts', items, ...(trace ? [{ __hfmOperationTrace: trace }] : [])),
  ...
  getPreviewBackend: (): Promise<'current' | 'directwrite-resident'> => invoke('fonts:getPreviewBackend'),
  cancelPreviewImage: (token: string): Promise<void> => invoke('fonts:cancelPreviewImage', token),
  renderPreviewImage: (item: FontItem, text: string, fontSize: number, width: number, height: number, token?: string): Promise<string> => invoke('fonts:renderPreviewImage', item, text, fontSize, width, height, ...(token ? [token] : [])),
