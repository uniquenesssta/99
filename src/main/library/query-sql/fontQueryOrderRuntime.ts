...
export function fontQueryOrderBy(request: FontQueryRequest): string {
  ...
  if (sortMode === "smart") {
    if (timeSortMode === "created")
      return "fonts.favorite DESC, (COALESCE(install_status.by_type, 'none') IN ('managed', 'both')) DESC, (COALESCE(install_status.installed, fonts.system_installed) = 1 AND COALESCE(install_status.by_type, '') <> 'managed') DESC, COALESCE(fonts.created_at, 0) DESC, fonts.file_name COLLATE NOCASE ASC, fonts.id ASC";
    return "fonts.favorite DESC, (COALESCE(install_status.by_type, 'none') IN ('managed', 'both')) DESC, (COALESCE(install_status.installed, fonts.system_installed) = 1 AND COALESCE(install_status.by_type, '') <> 'managed') DESC, fonts.modified_at DESC, fonts.file_name COLLATE NOCASE ASC, fonts.id ASC";
  }
  if (sortMode === "nameAsc")
