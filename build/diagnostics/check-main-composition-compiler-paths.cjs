...
function run(style, eol, mutation) {
  ...
  if (mutation) {
    ...
    assert(output.includes('128 compiler rejections; legacy omission reproduced; runtime unchanged'), output)
  }
}
...
if (caseIndex !== -1) {
  ...
  console.log('[diagnostics:main-composition-compiler-paths] forward/backslash/mixed paths with LF/CRLF passed; 128 compiler rejections retained; 3 regressions rejected')
}
