...
function harness({runtimePreload=false,cache=['a','b','c'],mode='success',throwLog=false,transforms={},partial=true,clock}={}) {
  ...
  function detail() {return load(renderer+'components/app/FontDetailPanel.tsx').FontDetailPanel({visible:true,selectedFont:library.fonts[select().selectedFontId] || visible.find(f=>f.id===selected.selectedFontId) || visible[0],selectedFontIds:selected.selectedFontIds,library,visibleFonts:visible,runFontCommand:command(),localTagSuggestions:[],sharedTagSuggestions:[]})}
  async function click(entry,view='grid',page='library',command='activate') {
    if(entry==='detail'||entry==='toolbar')button(entry==='toolbar'?panel(view,page):detail(),(process.argv.includes('--baseline')?'批量':'')+(command==='activate'?'激活':'取消激活')).props.onClick()
    else {
  ...
  function card(id,keys={},compact=false) {const i=interaction();hook=hookPort();hook.begin();const tree=load(renderer+'components/FontCard.tsx').FontCard({font:all.find(f=>f.id===id),compact,previewText:'test',onOpenDetail:e=>i.handleFontOpenDetail(e,all.find(f=>f.id===id)),onSelect:e=>i.handleFontSelect(e,all.find(f=>f.id===id))});tree.props.onMouseDown(event(keys));select();return tree}
...
async function run() {
  ...
  let cases=0
  for(const runtimePreload of [false,true])for(const view of ['grid','list'])for(const page of ['library','folders','tags'])for(const entry of ['detail','font','tag']) {
    ...
    const e=h.events.find(e=>e.stage==='entry');assert.equal(e.reason,{detail:'selection-toolbar',font:'font-context',tag:'tag-context'}[entry])
  ...
  // Actual card events with a deterministic clock, including the former double-click reselect.
  for(const compact of [false,true]) {
    let now=1000;const h=harness({clock:()=>now});
    h.card('a',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),['a']);assert(h.select().detailVisible)
    now=1099;h.card('a',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),['a'])
    now=1100;const card=h.card('a',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),[]);assert(!h.select().detailVisible)
    card.props.onDoubleClick?.(h.event());assert.deepEqual(plain(h.select().selectedFontIds),[])
    now=1199;h.card('a',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),[])
    now=1200;h.card('a',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),['a'])
    now=1201;h.card('b',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),['b'],'another font is not blocked')
    now=1400;h.card('c',{ctrlKey:true},compact);assert.deepEqual(plain(h.select().selectedFontIds),['b','c']);assert(h.select().detailVisible)
    now=1600;h.card('b',{},compact);assert.deepEqual(plain(h.select().selectedFontIds),['c']);assert(h.select().detailVisible)
    h.marquee(compact?'list':'grid');assert(h.select().detailVisible)
    assert(!treeNodes(h.panel(compact?'list':'grid')).some(n=>n.props?.className?.includes('selection-actionbar')))
    cases++
  }
  // The original U-08 selection handler leaves selection behind on the second click.
  {
    const file=renderer+'fontSelectionEventRuntime.ts',source=require('node:child_process').execFileSync('git',['show','0ea2635:'+file],{cwd:root,encoding:'utf8'})
    let now=1000;const h=harness({clock:()=>now,transforms:{[path.join(root,file)]:()=>source}});h.card('a');now=1100;h.card('a');assert.throws(()=>assert.deepEqual(plain(h.select().selectedFontIds),[]),assert.AssertionError);cases++
  }
  {
    const file=renderer+'components/FontCard.tsx',source=require('node:child_process').execFileSync('git',['show','0ea2635:'+file],{cwd:root,encoding:'utf8'})
    let now=1000;const h=harness({clock:()=>now,transforms:{[path.join(root,file)]:()=>source}});h.card('a');now=1100;const card=h.card('a');assert.deepEqual(plain(h.select().selectedFontIds),[]);card.props.onDoubleClick(h.event());assert.throws(()=>assert.deepEqual(plain(h.select().selectedFontIds),[]),assert.AssertionError);cases++
  }
  ...
  for(const view of ['grid','list']) {
    const h=harness({cache:[]});h.marquee(view);assert.deepEqual(plain(h.select().selectedFontIds),['a','b','c']);await h.click('detail',view)
  ...
  const partialCache=harness({cache:['a','b']});partialCache.marquee();await partialCache.click('detail');assert.deepEqual(plain(partialCache.requests),[['a','b','c']]);assert(partialCache.events.some(e=>e.reason==='selected:3.resolved:3.missing:0'));cases++
  ...
  for (const entry of ['detail','font']) {
  ...
  let cancelTime=1000;const cancelled=harness({clock:()=>cancelTime});cancelled.card('a');cancelTime+=100;cancelled.card('a');assert.deepEqual(plain(cancelled.select().selectedFontIds),[]);cases++
  const duplicate=harness();duplicate.select().setSelectedFontIds(['a','a','b']);await duplicate.click('detail');assert.deepEqual(plain(duplicate.requests),[['a','b']]);cases++
  ...
  pending.select().setSelectedFontIds(['a','b']);await pending.click('detail');assert.deepEqual([...pending.busy],['a','b']);await pending.command()('activate');assert.equal(pending.requests.length,1)
  ...
  const noReceipts=harness();noReceipts.handlers.set('fonts:activateFonts',()=>({message:'missing receipt'}));noReceipts.marquee();await noReceipts.click('detail');assert.equal(noReceipts.count,0);assert.equal(noReceipts.busy.size,0);assert.match(noReceipts.status.at(-1),/未确认 3 个/);cases++
  ...
  const f=path.join(root,renderer+'components/app/FontDetailPanel.tsx')
  ...
  broken.marquee();await broken.click('detail');assert.throws(()=>assert.deepEqual(plain(broken.requests),[['a','b','c']]),assert.AssertionError);cases++
