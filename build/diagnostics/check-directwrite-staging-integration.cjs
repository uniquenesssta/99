...
async function main(){
 ...
 const auth=load('src/main/path/fontPathAuthorizationRuntime.ts').createFontPathAuthorizationRuntime({fontExtensions:new Set(['.ttf']),readRoots:()=>[dir],watchedRoots:()=>[dir],appOwnedRoots:()=>[],fileSystem:fs.promises});
 const port=load('src/main/preview/native-renderer/directwriteFontStaging.ts').createDirectwriteFontStaging(command,auth.authorizeFontRead);
