...
async function main(){
 ...
 const allowed=path.join(dir,'allowed');fs.mkdirSync(allowed);
 ...
 try{
  ...
  const junction=path.join(allowed,'escape');require('node:child_process').execFileSync('cmd',['/c','mklink','/J',junction,outside]);
