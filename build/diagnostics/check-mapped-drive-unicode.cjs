...
function harness(transform = s => s) {
  ...
  const load = loader({
    ...
    [path.join(root, 'src/main/rust-core/rustCoreWorkerPathRuntime.ts')]: {resolveRustCoreWorkerPath: () => 'native-worker.exe'},
...
async function unicode(transform) {
  ...
  assert.equal(call.command,'native-worker.exe')
  assert.deepEqual(Array.from(call.args),['--mapped-drive-table'])
  ...
  assert.equal(h.calls.length,1)
  h.advance(30001)
  const fresh=r.mappedDriveTableAsync();await new Promise(resolve=>setImmediate(resolve));h.calls[1].done(null,encode([{drive:'Z:',remote:'\\\\nas\\新共享'}]));await fresh
...
async function failures() {
  for (const output of ['', 'broken',encode(null),encode({drive:'Z:',remote:'\\\\nas\\share'}),encode([{drive:'Z:',remote:'\\\\nas\\bad�'}]),encode([{drive:'Z:',remote:'local'}]),encode([{drive:'Z:',remote:'\\\\nas\\a'},{drive:'z:',remote:'\\\\nas\\b'}])]) {
    const h=harness(),task=h.runtime.mappedDriveTableAsync();await new Promise(resolve=>setImmediate(resolve));h.calls[0].done(null,output)
    ...
    h.advance(5001);const retry=h.runtime.mappedDriveTableAsync();await new Promise(resolve=>setImmediate(resolve));h.calls[1].done(null,encode([]));assert.equal((await retry).size,0)
  }
  const h=harness(),task=h.runtime.mappedDriveTableAsync();await new Promise(resolve=>setImmediate(resolve));h.calls[0].done(Error('timeout'))
  assert.equal(await task,null);h.advance(5001);const retry=h.runtime.mappedDriveTableAsync();await new Promise(resolve=>setImmediate(resolve));h.calls[1].done(null,encode([{drive:'Z:',remote:'\\\\nas\\中文'}]));await retry
...
async function main() {
  ...
  if(process.platform==='win32') {
    let observation
    const actual = loader({electron: {app: {getAppPath:()=>root}}, 'node:child_process': {...cp, execFile(command, args, options, done) {
