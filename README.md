...
- 2026-09-26：DW-02 实现专用 DirectWrite 常驻进程、版本/代次/输出身份校验、单活动任务与 64 项等待队列、相同请求合并、取消及期限后终止确认和有界退避；接入退出 freeze，父进程句柄看护与独立 stdin 线程处理绘制挂起时的退出。默认预览不变，无新增依赖。本地 typecheck/149 项诊断、构建与混淆通过；Windows 常驻绘制、挂起取消、父死/EOF 和源码变异门通过（36235481324）。追加唯一 owner 与清理失败停止准入边界，最终复验中。详见[试验任务书 §12](docs/plans/HFM_RESIDENT_DIRECTWRITE_PREVIEW_TASKBOOK.md#12-dw-02-执行边界进行中)。
