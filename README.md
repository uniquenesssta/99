...
- 2026-09-26：DW-04 实现隔离字体快照命令、授权/根代次准入、SHA-256 身份及本地 512MiB/128 项有界副本存储；单文件 64MiB，活跃租约保护，取消等待进程真实退出，独占临时文件及原子发布，按服务标记恢复残留。复用 Shared I/O 进程池，不增加依赖、不改激活目录或默认预览。每次仍在隔离进程校验源摘要，不宣称减少 NAS 读取；本地专项与构建/混淆通过，Windows 实际链路验收进行中，详见任务书 §14。

- 2026-09-26：完成 DW-03 原生字体对象复用：按路径/内容 SHA-256/源代次/face 建立有界 LRU，每项复用隔离 factory、内存 loader、face 与私有 collection，淘汰同时释放内部缓存；源文件读取后立即释放句柄，COM/D2D/WIC 会话跨请求保持。固定 128 项/256MiB 缓存及 512MiB 进程限额，常驻协议升级为 2 并核验摘要与真实资源指标；单次 CLI 和默认 UI 后端保持。Windows 1153 次请求验证复用、同 size/mtime 替换、文件释放与淘汰，1000 次切换私有内存稳定，大文件峰值约 308MiB；7 个 C++ / 4 个 TS 源码变异、原图像/取消/父死门均通过（Actions `36238266759`）。本地 typecheck/149 项诊断、构建/混淆通过；当前仍逐次校验本地字体内容，网络副本与界面入口留待 DW-04/05，不据此宣称端到端提速。详见[试验任务书 §13](docs/plans/HFM_RESIDENT_DIRECTWRITE_PREVIEW_TASKBOOK.md#13-dw-03-执行卡)。
...
- 2026-09-25：当前源码读取为 CRLF 的完整 verify（146 项）通过。Windows `36167830579` 通过受管卸载后，在真实 CIM 映射盘查询失败；前置实测 `36168691760` 确认原 1500ms deadline 在 1537ms 发出 SIGKILL（killed=true，stdout 0 字节，stderr 为空）。保留期限和严格成功断言；不能据此认定 CIM 命令/解析有错，也不能认定网络根离线。需要另行定位 PowerShell 启动与本机 CIM 查询耗时。验证链脚本修复已提交，但 Windows 硬门仍阻塞，C-04R/C-08.1-P 未推进。
...
- 2026-09-22：完成 C-06 退出结果三轴语义：退出 coordinator 不再用单一 `clean` 布尔值混合进程退出、持久化与本地清理，改为 `processExitClean / persistenceComplete / localCleanupComplete`，并记录 `cleanupRemaining / cleanupTimedOut / forced / reason`。C-05R pending temporary-font delete queue 的真实残留现在计入退出结果；planned residual 可正常退出但明确 `localCleanupComplete=false`。last-shutdown marker 保留 legacy `clean` 兼容，同时新增可选三轴字段；持久化失败不会写出 `clean:true`，下一次启动可区分 planned residual 与 crash recovery。五类结果（0 残留、1 残留已持久化、持久化失败、cleanup timeout、强制退出）均有行为门禁。C00 current 现为 1 缺陷 / 7 对照，C00-B04 已关闭，仅剩 C00-B05 对应 C-07。Windows C-06 verification `35630687836` 的 typecheck、C00 current、bounded-local-exit、shutdown-log-durability、window-close-flush、main application/composition、Electron/Vite build、混淆和 diff check 全绿。另已根据 2026-09-21 Windows 实机日志确认 C-05/C-05R 的“激活 → 取消 → 再激活 → 再取消”链路真实通过。下一项 C-07。

- 2026-09-21：完成 C-05R Windows 临时字体占用退避回收：`os error 32` sharing violation 不再被高频重复删除；持久删除记录新增 sharing-blocked/nextRetryAt，自动重试采用 5 秒、15 秒、60 秒、5 分钟、15 分钟封顶退避，冷却期重复 flush 不再触碰文件，启动恢复与用户“重试清理”仍可显式立即重试。删除前仍复用 C-05 managed path + file identity ownership 核验，激活入口修复保持不变；取消激活可在 resource/registry 已结算后让暂时被 Windows 占用的文件留在 durable delete queue 后台回收。Windows typecheck、activation-entry、managed recovery（含 sharing violation 正例）、C-05 batch settlement、Electron/Vite build、混淆与 diff check 全绿（CI 35591586994）。C-05R 自动门与后续 Windows 实机链均已通过；退出语义由 C-06 继续收口。

- 2026-09-21：修复 C-05 收尾后的实机激活入口回归：Renderer 曾把 `systemInstalled/isInstalled` 的缓存状态当成激活 admission，导致取消激活后状态尚未刷新时“激活”按钮仍显示，但目标被过滤成 0、完全不发送 `fonts:activateFonts`。现仅由 Renderer 排除明确 active、Windows 默认受保护字体和 busy 项，是否永久安装统一交由 main `activateFontSessionTransaction()` 权威判断；C-05 Rust ownership 协议未改。新增 stale-installed 正例与重新加入 `!isInstalled` 的退化反例，Windows typecheck、activation/font-command、managed recovery、C-05 deactivation settlement、Electron/Vite build、混淆和 diff check 通过（CI 35590647591）。后续 Windows 实机日志已确认“激活 → 取消激活 → 再激活 → 再取消激活”成功；`os error 32` 占用由 C-05R 退避回收处理。
